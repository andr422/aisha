...
function renderTop(who, role) {
  // В базе роль владельца называется owner — она шире супервизорской и
  // пригодится, когда появится управление учётными записями. На экране
  // подписываем её «adm»: со вторым супервизором в центре два одинаковых
  // ярлыка при разных правах сбивали бы с толку.
  const ROLE_NAME = { supervisor: "супервизор", owner: "adm", therapist: "терапист" };
